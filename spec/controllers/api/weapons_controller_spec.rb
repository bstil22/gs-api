...
RSpec.describe Api::WeaponsController, type: :controller do
  context 'with an invalid or nil token' do
    it 'returns a 401 if request does not contain token' do
      get :index
      expect(response.status).to be(401)
    end

    it 'returns a 401 if request contains an invalid token' do
      request.headers['HTTP_AUTHORIZATION'] = "Bearer TRASH TOKEN"
      get :index
